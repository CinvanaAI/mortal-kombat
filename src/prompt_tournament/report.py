...
def render_report(result: dict) -> str:
    ...
    table=''.join(f'<tr><td>{esc(r["rank"])}</td><th scope="row">{esc(r["model_id"])}</th><td>{esc(r["status"])}</td><td>{esc(r.get("reason",""))}</td></tr>' for r in ranks)
    candidates=[]
